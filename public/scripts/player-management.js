...
async function loadPlayers() {
  try {
    // Using our Firebase service to get players
    players = await firebaseService.getAllPlayers();
...
function setupPagination() {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    pageLink.addEventListener('click', (e) => {
      ...
      currentPage = i;
      renderPlayers();
      setupPagination(); // Update active page
    });
...
async function editPlayer(playerId) {
  try {
    ...
    if (newRating !== undefined) {
      ...
      await loadPlayers(); // Refresh data
    }
  } catch (error) {
